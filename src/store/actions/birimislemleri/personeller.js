...
export const addToPersoneller = personel => async dispatch=>{
    ...
    try{
        const res =await axios.post('https://localhost:44312/Personeller/AddNewPersonel',personel);
        dispatch({
...
export const removeFromPersoneller = personel => async dispatch=>{
    try{
        const res =await axios.post('https://localhost:44312/Personeller/DeleteaPersoneller',personel);
...
export const getPersonelData = birimId =>async dispatch=> {
    try{
        const res =await axios.get(`https://localhost:44312/Personeller/GetListofPersoneller?BirimId=${birimId}`)
        dispatch({
